"""Build Appy into a single .exe, then wrap it in an installer:

    python tools/build_exe.py

Produces dist/Appy.exe (PyInstaller) and dist/AppySetup.exe (Inno Setup, if
...
def main() -> int:
    ...
    print(f"[1/2] PyInstaller, Appy {__version__}; this takes a few minutes")
    code = subprocess.run(cmd, cwd=ROOT).returncode
    if code:
        return code
    exe = ROOT / "dist" / "Appy.exe"
    ...
    code = subprocess.run([str(ISCC), "/Q", f"/DAppVersion={__version__}", str(ROOT / "installer" / "appy.iss")], cwd=ROOT).returncode
    if code == 0:
        setup = ROOT / "dist" / "AppySetup.exe"
