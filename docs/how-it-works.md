...
Appy speaks **AFC** to the device and **WebDAV** to Explorer, and translates
between the two.

```
iPhone / iPad ──USB──▶ usbmuxd ──▶ pymobiledevice3 (AFC) ──▶ Appy ──WebDAV──▶ File Explorer
...
- **FTP** downloads files correctly but never feeds them into Explorer's
  ...
  fixed server-side. An early version of Appy used FTP.
