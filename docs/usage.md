...
3. Open Appy. The device appears in the list within a couple of seconds.
...
Appy shows the device's media storage, the same area iTunes and Finder expose:
...
Appy. Explorer will refuse any such operation. Viewing and copying files **off** the device is the only direction supported.
...
The address is only valid while Appy is running and the device is plugged in.
It binds to `127.0.0.1`, so nothing outside this machine can reach it.

## Disconnecting

Unplug the device or close Appy. Either stops the server; Explorer windows
pointing at it will show an error on their next refresh, which is expected.

If the device auto-locks mid-transfer, Appy reconnects automatically. A brief
pause is normal.
