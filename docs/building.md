...
- `dist/Hudzen.exe` — one file, no Python needed on the target machine
- `dist/HudzenSetup.exe` — if [Inno Setup](https://jrsoftware.org/isinfo.php) is
...
`%LocalAppData%\Programs\Hudzen`. The target machine still needs the Apple
...
with generated notes and `HudzenSetup.exe` attached. Nothing to tag by hand.
...
Installed copies check `https://api.github.com/repos/jaym-01/Hudzen/releases/latest`
...
silently. The installer closes Hudzen, replaces it, and relaunches it.

Two constraints:

- The asset must be named `HudzenSetup.exe` and its download URL must sit under
  `github.com/jaym-01/Hudzen/releases/download/`. Any other URL is ignored, so a
