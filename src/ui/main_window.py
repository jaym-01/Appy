...
from ..device import POLL_MS, Device, scan
...
class MainWindow(QMainWindow):
    ...
    def _start_server(self, udid: str) -> None:
        ...
        server = DeviceWebdav(self.bridge, udid, next(p for p in itertools.count(BASE_PORT) if p not in used))
        try:
            server.start()
        except Exception:
    ...
    def _on_update_check(self, future) -> None:
        ...
        if found:
            version, url = found
            self.update_label.setText(f"Appy {version} is available")
    ...
    def _on_update_installed(self, future) -> None:
        try:
            ...
            return
        self.close()  # the installer is running; it relaunches Appy when done
