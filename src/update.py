...
def _get(url: str, accept: str = "application/vnd.github+json"):
    return urllib.request.urlopen(
        urllib.request.Request(url, headers={"User-Agent": "Hudzen", "Accept": accept}), timeout=15
    )
...
async def install(url: str) -> None:
    ...
    quits; /CLOSEAPPLICATIONS lets Setup close Hudzen itself if it is still up,
